    ...
    Ok(ShaderProgram(Rc::new(ShaderProgramInner {
        normal: ShaderProgramInternal {
            ...
            additional_uniforms: additional_uniforms
                .iter()
                .filter_map(|uniform| {
                    let name = CString::new(uniform.name.as_bytes()).ok()?;
            ...
            texture_uniforms: texture_uniforms
                .iter()
                .filter_map(|name_| {
                    CString::new(name_.as_bytes()).ok().map(|name| {
                        let location = gl.GetUniformLocation(program, name.as_ptr());
        ...
        debug: ShaderProgramInternal {
            ...
            additional_uniforms: additional_uniforms
                .iter()
                .map(|uniform| {
                    let name = CString::new(uniform.name.as_bytes()).unwrap_or_default();
            ...
            texture_uniforms: texture_uniforms
                .iter()
                .filter_map(|name_| {
                    let name = CString::new(name_.as_bytes()).ok()?;
                    let location = gl.GetUniformLocation(debug_program, name.as_ptr());
                    Some((name_.to_string(), location))
                })
                .collect(),
        },
...
impl RenderElement<GlesRenderer> for ShaderRenderElement {
    fn draw(
        ...
        let Some(shader) =
            Shaders::get_from_frame(frame).and_then(|shaders| shaders.program(self.program))
        else {
            return Ok(());
        };
