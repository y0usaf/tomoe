...
(defstruct mounted spec state effects (dispatches 0))
(defstruct runtime backend socket sources mounts windows outputs effective
  (running t) processes last-error (generation 0))
...
(defun invoke-extension (mounted context event)
  ...
  (let ((spec (mounted-spec mounted)))
    (handler-case
      ...
      (serious-condition (condition) (error "Extension ~A: ~A" (spec-name spec) condition)))))
    ...
    (loop for round below 16 do
      (dolist (mounted candidate)
        (when (or (member (spec-name (mounted-spec mounted)) force :test #'equal)
          ...
          (setf commands (nconc commands (invoke-extension mounted context event)))))
    ...
    (ecase (getf event :type)
      ((:map :metadata)
       (let* ((id (getf event :id))
              ...
              (new (loop for key in '(:id :title :app-id :width :height)
                         append (list key (getf event key)))))
      ...
      (:unmap
       (setf (runtime-windows runtime)
             (remove (getf event :id) (runtime-windows runtime) :key (lambda (w) (getf w :id))))
       (push :windows changed))
      (:outputs
                ...
                (loop for m in (runtime-mounts runtime) for spec = (mounted-spec m)
                      collect (list :name (spec-name spec) :source (spec-source spec)
                                    ...
                                    :effects (loop for e in (mounted-effects m)
                                                   collect (cons (effect-kind e) (effect-arguments e))))))))
